...
import os
# 需要排除的文件夹列表
exclude_folders = ['.git', '.idea', '.vscode', 'venv', '__pycache__', 'structure.py']
...
def generate_project_structure(directory, indent='', is_last=False, is_root=False, output_file=None):
     ...
     for i, item in enumerate(items):
         ...
         if os.path.isdir(item_path):
             ...
             generate_project_structure(item_path, new_indent, is_item_last, is_root=False,output_file=output_file)
         else:
             # 如果是文件，则添加文件标记
             marker = '└── ' if is_item_last else '├── '
             output_file.write(f"{indent}{marker}{item}\n")
...
if __name__ == '__main__':
     ...
     # 生成项目结构并写入文件
         generate_project_structure(r'E:\LK\NLPProject\TONALITYLABEL\model_dev', is_root=True, output_file=file)
